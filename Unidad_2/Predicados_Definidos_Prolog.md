# Predicados Definidos por Prolog
**Orozco Mora Jorge Alexander**
*NC: 22120703*

---
**member/2**: Verifica si un elemento pertenece a una lista.
~~~
member(X, [X|_]).
...
**nth0/3**: Devuelve el elemento en la posición N (empezando desde 0).
~~~
nth0(0, [X|_], X).
...
**last/2**: Obtiene el último elemento de una lista.
~~~
last([X], X).
last([_|T], X) :- last(T, X).
~~~

**sum_list/2**: Suma todos los elementos numéricos de una lista.
~~~
sum_list([], 0).
sum_list([X|T], S) :- sum_list(T, S1), S is X + S1.
~~~

**max_list/2**: Obtiene el máximo valor de una lista.
~~~
max_list([X], X).
max_list([X|T], M) :- max_list(T, M1), M is max(X, M1).
~~~

**sublist/2**: Determina si una lista es sublista de otra.
~~~
